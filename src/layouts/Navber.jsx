      ...
      <nav className="mx-auto flex max-w-7xl items-center justify-between px-6 py-4">
        ...
        <ul className="hidden items-center gap-8 md:flex">
          {navLinks.map((link) => (
              <li key={link.label}>
                <Link
                  to={link.to} onClick={(e) => link.isSpecial && handleAIClick(e, link.to)} 
                  className={`text-sm font-medium uppercase tracking-wide transition-all ${
                    link.isSpecial 
                      ? "bg-linear-to-r from-red-600 to-orange-500 px-4 py-2 rounded-full text-white shadow-lg shadow-red-600/20 hover:scale-105 animate-pulse"
